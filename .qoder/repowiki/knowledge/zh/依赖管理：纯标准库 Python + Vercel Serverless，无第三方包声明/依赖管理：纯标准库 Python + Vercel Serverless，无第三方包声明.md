...
- Vercel Serverless 函数位于 `api/` 下的 7 个路由，RSS 全文接口使用项目声明的 `jsdom` 与 `@mozilla/readability` 运行时依赖；其余函数通过原生 `fetch` 调用外部 API。函数清单以 `vercel.json` 为准。
...
| `vercel.json` | Vercel 配置，声明 7 个 Serverless Function 及其超时限制 |
| `.github/workflows/update.yml` | GitHub Actions 分层定时任务（UTC 21:00 full、UTC 2/6/10/14 incremental）+ `workflow_dispatch` |
| `api/refresh.js` / `api/rss.js` / `api/article.js` | Vercel API：刷新触发、RSS 快照/T1 实时数据和全文兜底 |
| `build_rss_aggregator.py` | RSS 构建器，维护 711 源、72 小时历史和静态 chunk |
...
- **无版本锁定**：Python 环境由 `actions/setup-python@v6` 固定为 `3.11`；Node.js 通过 `npx --yes vercel` 拉取最新 CLI，无版本约束；全文 API 的 `jsdom` 与 `@mozilla/readability` 由 Vercel 运行时提供。
...
- **并发安全**：Workflow 使用 `concurrency.group: starhub-update` 且 `cancel-in-progress: true`；构建前同步 `origin/main`，push 使用 `--force-with-lease` 保护远端更新。
- **全文跨域约束**：`api/article.js` 允许 GET/OPTIONS 并返回 `Access-Control-Allow-Origin: *`，GitHub Pages 必须调用 Vercel 绝对 URL。
