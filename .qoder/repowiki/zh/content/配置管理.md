...
## 更新摘要
**当前配置状态（2026-09-06）**
- Star 主线配置：分类映射、中文描述缓存、Trending 基线。
- RSS 主线配置：711 个信源，T1=23、T2=20、T3=668，72 小时历史和增量状态。
- Vercel 配置：refresh/search/events/news/rss/article/agihunt 共 7 个函数。
- 安全约束：`GH_TOKEN`、`REFRESH_KEY`、`VERCEL_TOKEN` 只通过受控 Secret 注入，文档不记录实际值。
...
## 简介
本仓库包含 Star 收藏主线和 RSS 聚合主线，配置由 Python/JSON 数据文件、GitHub Actions、Vercel 函数和静态产物共同组成。本文既说明 Star 主线的分类、描述、Trending 配置，也说明 RSS 的信源分层、72 小时历史、翻译缓存、API 和部署配置。
...
- `rss_sources.json` / `rss_history.json` / `rss_api_snapshot.json`：RSS 信源、历史与快照
- `translations.json`：Star/RSS 构建与 API 共享翻译缓存
- `vercel.json`：7 个 Serverless 函数及超时配置
- `README.md`：项目说明与在线地址
...
## 架构总览
整体流程分为 Star 配置链和 RSS 配置链：
- GitHub Actions 由 schedule 或 `workflow_dispatch` 触发；UTC 21:00 为 RSS full，其余 UTC 2/6/10/14 为 incremental。
- Star 主线读取 `known_categories.json`、`descriptions_zh.json` 和 `trending_snapshot.json`，生成 `index.html`。
- RSS 主线读取 `rss_sources.json`，维护 `rss_history.json` 和 `rss_api_snapshot.json`，生成 RSS 页面与 chunk；T1 运行时刷新由 `/api/rss` 负责。
- `translations.json` 由构建侧生成并供 API 层复用。
- Vercel 环境变量中的密钥只保存在 Secret，不写入 Wiki 或脚本；具体 CORS、API 和部署约束以 `HANDOFF.md` 为准。
...
- 触发机制
  - 定时：GitHub Actions 使用 UTC 21:00 full 与 UTC 2/6/10/14 incremental，并支持 workflow_dispatch；普通 push 不触发。
