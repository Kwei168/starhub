...
- 通过 GitHub Actions schedule/`workflow_dispatch` 触发统一构建，将 Star 主站、AI 晨报和 RSS 产物提交到仓库并部署到 GitHub Pages/Vercel。

该脚本仅依赖 Python 标准库，无需第三方包，便于在 GitHub Actions 环境中稳定运行。

### 当前 RSS 数据链路（2026-09-06）

- `build_rss_aggregator.py` 管理 711 个信源：T1=23 高频源、T2=20 中频源、T3=668 低频源；T1 由 `/api/rss?refresh=1` 实时抓取，T2/T3 由 Actions 增量构建并写入快照。
- `rss_history.json` 按 link 去重并维护 72 小时滚动窗口；`rss_api_snapshot.json` 保存累积数据和 `meta.last_fetch`。
- chunk 字段契约：构建产物提供 `full_content`，前端 `buildArt()` 统一映射为 `fc = it.fc || it.full_content || ''`；图片字段映射到 `img`。
- RSS 页面数据流为 `rss-data-0.js` 首屏块 → `rss-data-1.js` 后台块 → `SOURCES` → `buildArt()` → `renderWall()`；`sourceKey|link` 去重，缺图时回退纯文字卡。
- `/api/rss` 快照优先；`/api/article` 先查快照全文 map，再按 GitHub/YouTube/Readability 分支实时提取，并允许 GitHub Pages 跨域 GET/OPTIONS。

**章节来源**
- [README.md:1-22](file://README.md#L1-L22)
- [HANDOFF.md:8-44](file://HANDOFF.md#L8-L44)

## 项目结构
- fetch_and_build.py：Star 主线核心脚本，负责 Star、Trending、分类、翻译、事件聚合与页面构建。
- build_rss_aggregator.py：RSS 主线核心脚本，负责信源分层、抓取、历史累积、翻译、快照、chunk 和 RSS 页面。
- .github/workflows/update.yml：GitHub Actions 工作流，定义 full/incremental 调度、提交和 Vercel 部署步骤。
- README.md：面向最终用户的说明与在线地址。
- HANDOFF.md：接手文档，包含架构要点、注意事项与本地运行指引。
...
- [HANDOFF.md:18-44](file://HANDOFF.md#L18-L44)
...
**章节来源**
- [HANDOFF.md:48-54](file://HANDOFF.md#L48-54)
...
- [fetch_and_build.py:489-560](file://fetch_and_build.py#L489-L560)
- [fetch_and_build.py:444-451](file://fetch_and_build.py#L444-L451)
- [HANDOFF.md:58-68](file://HANDOFF.md#L58-68)
- [HANDOFF.md:72-79](file://HANDOFF.md#L72-79)
