...
- **增强** 并发控制：使用 concurrency 组防止重复执行，cancel-in-progress=true 确保当前任务完成。
...
CA->>PY : 触发更新(UTC 21:00 full 或 UTC 2/6/10/14 incremental)
...
- **频率优化**：从每天更新改为按分层 schedule 更新（UTC 21:00 full、UTC 2/6/10/14 incremental），确保数百个仓库的星标数保持最新
- **并发控制**：使用 `concurrency.group: starhub-update` 防止重复执行，`cancel-in-progress: true` 确保当前任务完成
- **权限最小化**：仅授予 `contents: write` 权限，降低安全风险
- **增量提交**：仅在数据变化时提交更改，减少版本历史噪音
- **冲突解决**：构建前同步 `origin/main`，push 使用 `--force-with-lease` 保护远端更新
- **Vercel 集成**：部署完成后自动触发 Vercel 重新部署，加速更新传播

```mermaid
flowchart TD
Start(["开始"]) --> Schedule{"定时触发<br/>UTC 21:00 full 或 UTC 2/6/10/14 incremental"}
...
- **频率优化**：UTC 21:00 full 或 UTC 2/6/10/14 incremental 触发 update 作业，确保数据实时性
- **并发控制**：使用 concurrency 组防止重复执行，cancel-in-progress=true 确保任务完整性
...
- **增强** 持续维护周期：UTC 21:00 full 或 UTC 2/6/10/14 incremental 触发更新
...
### 自动化配置参数
- **调度频率**：UTC 21:00 full 或 UTC 2/6/10/14 incremental 触发
