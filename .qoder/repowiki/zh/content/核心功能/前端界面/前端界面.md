...
## 当前双页面前端架构（2026-09-06）

### Star 主站 `index.html`
- 由 `template.html` 和 `fetch_and_build.py` 生成，保留原有 Star 收藏、搜索、分类/语言/星标筛选、排序、收藏置顶、AI 排行榜、关注动态和主题切换。
- 该页面的主站侧边仪表盘和事件流属于 Star 主线，不应与 RSS 页面 `.wall` / `.ai-feed-panel` 混淆。

### RSS 聚合页 `rss-aggregator.html`
- 由 `build_rss_aggregator.py` 生成，数据来自 `rss-data-0.js` 首屏块、`rss-data-1.js` 后台块和运行时 `/api/rss`。
- ≥1280px：`.wall-wrap` 使用 flex，`aside.ai-feed-panel` 位于左侧，`position:sticky`、宽 300px、独立滚动；`#btnAiFeed` 和关闭按钮隐藏。
- `<1280px`：AI 面板保持 fixed 右侧抽屉；移动端通过 `#btnAiFeed` 打开，`scrim` 提供遮罩，≤700px 宽度为 `100vw`。
- reader2 全文来源依次为内嵌 `a.fc`、页面 `_articleCache[url]`、绝对 Vercel `/api/article`；chunk 的 `full_content` 通过 `fc = it.fc || it.full_content || ''` 兼容。
- `buildArt()` 按 `sourceKey|link` 去重；无图文章不带 `.cover-card`，恢复紧凑文字卡。桌面侧栏改动不能破坏 reader2、src-panel（z-index 80）和 share-modal（z-index 100）。

### 跨域与验证
- GitHub Pages 上所有 Serverless 调用使用绝对 Vercel URL；`/api/article` 必须允许 GET/OPTIONS，并返回 `Access-Control-Allow-Origin: *`。
- RSS 页面改动验证 `verify_online_sidebar.cjs` 和 `verify_ai_sidebar_e2e.cjs`；Star 主站改动仍需本地产物构建和原有搜索/筛选/收藏回归。
