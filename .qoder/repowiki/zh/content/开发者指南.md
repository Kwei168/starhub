...
### Star 收藏主线
- `fetch_and_build.py` 负责 starred repos、搜索、following/events、分类、翻译、Trending 和 Star 主站模板渲染。
- `template.html` 注入 `__DATA__`、`__CATS__`、`__LANGS__`、`__FAVS__`、`__TRENDING__`、`__FEED__`、`__UPDATED__`，生成 `index.html`。
- 主站功能边界包括收藏置顶、搜索、分类/语言/星标筛选、排序、主题切换、导入导出、AI 排行榜和关注动态。

### RSS 聚合主线
- `build_rss_aggregator.py` 管理 711 个信源（T1=23、T2=20、T3=668），生成 72 小时历史、快照、首屏/后台数据块和 RSS 页面。
- RSS 页面在 ≥1280px 时通过 `.wall-wrap` 将 `aside.ai-feed-panel` 作为左侧 `sticky` 常驻栏；窄屏保持右侧 fixed 抽屉。
- `buildArt()` 使用 `sourceKey|link` 去重；无图片时不输出 `.cover-card`，回退到紧凑文字卡。
- 全文优先使用 `a.fc`，兼容 chunk 的 `full_content`；其次使用页面缓存，最后访问绝对 Vercel `/api/article`。

### 维护边界
- `index.html` 和 RSS HTML/chunk 都是自动生成文件，修改必须回到对应 Python 源文件。
- GitHub Pages 是用户入口，Vercel 提供 API 和 production deployment；相对 `/api/...` URL 在 GitHub Pages 上会 404。
- 涉及 RSS/API 的改动需验证源码 workflow run、dynamic 连锁 run、线上静态断言和 Playwright `1440×900` / `390×844` 双视口；Star 主线仍需验证 `index.html` 构建结果和主站交互。
...
- `template.html`：Star 主站前端模板（CSS/JS 内联），使用占位符注入数据
- `index.html`：Star 主站构建产物（由脚本生成，勿手动修改）
- `rss-aggregator.html`、`rss-data-0.js`、`rss-data-1.js`：RSS 自动生成产物（勿直接修改）
- known_categories.json：已知项目的稳定分类映射
- descriptions_zh.json：非中文简介的中文翻译缓存
- trending_snapshot.json：星标快照，用于计算日增量
- .github/workflows/update.yml：GitHub Actions 分层调度、提交和 Vercel 部署流程
- build_rss_aggregator.py：RSS 三层信源、72 小时历史、卡片墙、AI 动态面板和 reader2 生成器
- api/article.js / api/rss.js：RSS 全文兜底和分层数据 API
- rss_sources.json / rss_history.json / rss_api_snapshot.json：RSS 信源元数据、历史与快照
- README.md：面向最终用户的使用说明
- HANDOFF.md：接手文档，含架构要点与注意事项
...
- [HANDOFF.md:18-29](file://HANDOFF.md#L18-L29)
...
**章节来源**
- [HANDOFF.md:58-69](file://HANDOFF.md#L58-L69)
- [dev_render.py:28-46](file://dev_render.py#L28-L46)
...
## Wiki 同步维护约定
- 代码或架构变更涉及 Star 主线、RSS 主线、API、部署或验证流程时，同步更新对应 Wiki 内容与 `HANDOFF.md`。
- 每次 Wiki 调整后运行 `python check_wiki_sync.py`；脚本会核对 starred/search/following/events、分类/翻译/Trending/收藏、RSS 711 源分层、全文 CORS、AI 侧栏、workflow 调度、7 个函数、失效文件引用、旧事实和敏感令牌。
- 该检查是轻量一致性检查，不直接作为 CI 阻断门禁；发布前仍需结合 `git diff --check`、Markdown 结构检查和页面/API 回归。

## 版本管理与发布策略
- 自动化发布：Actions 在 UTC 21:00 执行 full，在 UTC 2/6/10/14 执行 incremental，生成并提交 Star/RSS 产物
- 并发控制：concurrency 组 starhub-update，cancel-in-progress=true，新触发取消旧排队任务
...
**章节来源**
- [.github/workflows/update.yml:1-41](file://.github/workflows/update.yml#L1-L41)
- [HANDOFF.md:48-55](file://HANDOFF.md#L48-L55)
