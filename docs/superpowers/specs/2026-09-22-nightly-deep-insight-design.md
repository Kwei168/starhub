...
结论要改口：**批 3.14 的现网效果测不出来**，不是我上一版写的"方向对了"。
"每条挂几篇"在场 39（还没上这批改动）就已经是 2；9k 字档的"压"场 41 也不比 38/39 高；
四夜之间这些数的起伏都在"一夜一个样本"的量级里，而 qualified 已经掉到 0。
所以 #75 现在**没有一条支持它的现网证据**，只有一条反对它的：`dropped` 第一次到 4（两条），
而 `coverage_blocked=2` 恰好就是那两条 —— prompt 要求变严之后，凑上来的弱配对被核查摘掉，
把本来能达标的两条打了下去。这一条还不足以判"回滚"，但足以判"别再往同一个方向加要求"。
