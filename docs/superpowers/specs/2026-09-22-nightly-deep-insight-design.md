...
**抬起的那一半 / 没抬的那一半（这一段的初版写错了，见下面的更正）**：初版我写"9k 字档压在几篇从 6 抬到 7~8"，
并且把 `#68 验收` 那栏的"5 条 引=压"当成场 41 的数。**两处都不成立**：
把四夜一起用同一个探针重算（`_scratch/read_b75.py` → `b75_readout.txt`，口径与判据同源：
`压`=claims evidence 去重 ∩ citations、长度按 `count_chars` 取裁前原长），我自己的数是——

| 夜 | 长文数 | 每条挂几篇(中位) | 压在几篇(中位) | 9k 字档的"压" | dropped 最大 | qualified |
|---|---|---|---|---|---|---|
| 38（#74 生效后） | 8 | 1 | 7.5 | 6,6,7,8,8,8,9 | 3 | 2 |
| 39 | 8 | **2** | 7.0 | 6,7,7,8,9 | 1 | 1 |
| 40 | 8 | 1 | 6.0 | 3,6,6,7,7,8 | 2 | 1 |
| 41（含批 3.14） | 7 | 2 | 7.0 | 5,6,6,7,7 | **4** | **0** |

结论要改口：**批 3.14 的要求被执行了，但没换来达标**。
把每条 claim 单独拎出来数（`_scratch/read_b75_split.py` → `b75_split.txt`，
"每条挂几篇"是**逐条 claim** 的中位与最小值，不是条目级中位）：

| 夜 | claims 样本数 | 每条挂几篇 中位 / 最小 | 压(中位) | dropped 最大 | 结构字段整组为空的长文 |
|---|---|---|---|---|---|
| 37 | 38 | 1.0 / 1 | 6.0 | 5 | 0 |
| 38 | 63 | 1 / 1 | 7.5 | 3 | 0 |
| 39 | 69 | 2 / 1 | 7.0 | 1 | 0 |
| 40 | 59 | 1 / 1 | 6.0 | 2 | 1（`evt_20260925_011`） |
| 41（含批 3.14） | 51 | **2 / 2** | 7.0 | **4（两条）** | 0 |

- **要求真的落地了**：场 41 的 51 条 claim 里**没有一条只挂 1 篇**（min=2），
  前四夜每一夜都有大量 1 篇的 claim（场 40 的 59 条里 48 条是 1 篇）。
  我上一版写"效果测不出来"是**过度更正** —— 机制层面这一刀完全起效，看得见、可复算。
- **但达标数没动**：`压` 的中位 7.0 与场 38/39 的 7.5/7.0 同档，`qualified` 反而是 0；
  也就是"每条多挂一篇"挂的是**同一批两篇**，去重后并不铺开 —— 这正好是 #75 原本要说清的
  "铺开 vs 条数"的另一半：**光要求每条 2 篇，得到的是配对复用，不是覆盖面**。
- **代价是真的**：`dropped` 第一次出现两条 4（前四夜最大 1~3），而那两条正是 `压` 最低的两条（5 与 6）。
...
4. 这批**先不撤回也不加码**：撤回的理由（"回到压在 6 篇那一格"）我自己复算时已经推翻 ——
   场 39 没上这批改动也做到过"每条挂 2 篇、压 7 篇"，所以留在原位不动，
   但**停止沿同一方向继续加要求**；下一步只走两条待拍板的路（上面第 3 条），
   或者拿"同一条 item 第二趟是否还犯同一条"这种能同夜自比的口径重做验收。

（推送与现状：`main` 与 `insight-verify` 都已同步（blob 对账"需要推的文件 0 个"），
cron 已在 main 上生效、首夜自动 publish 在 2026-09-25 20:00Z（北京 04:00）触发；
白天构建在跑时工具的守卫确实拒绝过一次推送（"会被它的 Commit 步骤回滚"），我没有用 `--allow-running` 绕过，而是等窗口。）

### 10.61 第四轮审查（读 3.14/3.15）：一处我写错的结论、一处设计债、一条我驳回的 P1

审查员自己重算了四夜的数，并且**推翻了我 §10.60 初版的核心结论**——我按它的算法自己复跑一遍
（`_scratch/read_b75.py` → `b75_readout.txt`），它是对的、我是错的：场 39 在批 3.14 之前
"每条挂几篇"就已经是 2，9k 档的"压"场 41 也不比 38/39 高。所以那一段已经就地改成
**"批 3.14 的现网效果测不出来"**，并把 #75 的下一步从"再加一层要求"改成
"要么换验收口径（同一条 item 第二趟还犯同一条），要么走 #71/#75 那两个待拍板的口子"。
教训不是"审查员更聪明"，而是我又一次拿一夜的读数当趋势写进台账 —— 与 §10.51 同一类错。

**同一节还要第二次改口（这次的错在我自己）**：我据此写下"批 3.14 的现网效果测不出来"，
把条目级的"压"当成唯一指标，于是看它没动就判"没效果"。再按**逐条 claim** 数一遍
（`_scratch/read_b75_split.py`）才发现：场 41 的 51 条 claim **没有一条只挂 1 篇**，
而前四夜到处都是 1 篇的 claim（场 40 的 59 条里 48 条是 1 篇）。
所以正确的说法是**"要求落地了、达标数没动、代价(dropped)上升"**，不是"没效果"。
两次改口是同一个毛病的两面：**没先把指标定义清楚就开始下结论** ——
"铺开"这个概念既可以在条目级量（压几篇）、也可以在论断级量（每条挂几篇），
换个口径结论就能翻面。以后这类读数一律两栏一起给，缺一栏就不许写结论。

办掉的三条：
- **P1-A（设计债，认）**：`narr_out` 的归属是靠代码块位置切片的，以后有人在块中间插一条结构级判据
  就会静默变成"段落级"。我不改切片写法（改了就要动 28 处 `fails.append`），
  改成**把归属钉成明账**：`test_the_narrative_group_is_pinned_by_name_not_by_position`
  正面要求"长度/条目体必须在组内"，并反向钉住 `quality.basis` 与"只压在 N 篇"两条不许进来。
  变异体 K1 现在同时被这条新判据抓到（红 2 条），证明它不是摆设。
- **P1-D（认，且我上一版数字给错了）**：两条覆盖要求同时喊时必须给出**算得出来**的最小条数。
  原来那句写"本项下限 3 条"，而 ask=7 时 3 条 ×2 篇结构上就到不了 7 —— 等于把够不着的要求
  又原样复述一遍。现在按 `⌈ask/CLAIM_EVID_MIN⌉` 打印（ask=7 → "论断至少写 4 条"），
  配 K4（不说明）与 K5（用常量下限糊）两具变异体。
- **P2（认）**：`section_extra` 的标题原来固定写"正文契约条目 + 判分分项"，而契约没过那一趟
  判定根本没跑 ⇒ 标题对内容撒谎。现在按实际拼出的两组分别落标题。

驳回的一条 **P1-C**（它建议 `sec_extra` 为空时回退到整份 `extra`）：
`section_extra == ""` 只发生在"违约全是整条级、判定也没跑"的那一趟，此时段落确实无处可改；
回退到整份 `extra` 等于把第三轮刚关掉的那条通道（模型自报字符串进正文提示词）重新打开，
并且会制造"这一段能修 quality.basis"的假希望。保留空串，理由写在这里。

审查员另外提的两条 P2 我没动、只记账：① `primaries` 只喂给**补结构那一趟**，
逐段成文时还没看到段落归属 —— 那才是 #75"写时铺不开"的正身，要做就得动 `build_section_prompt`
的输入（改的是上下文组装，不是 prompt 措辞），我把它并进 #75 的第二刀一起决策；
② 两处 citation 下界公式不同形（`_structure_rules` 的 `max(min(CITE_MIN,supply),ask)`
...
**整表复跑读数（12:07，`mut_final_run3.txt`）**：第一次基线红是隔离树缺 `template.html`（新加的 nav 判据读它）—— harness 的旁路文件清单没跟上判据读的新文件；补进 `EXTRA_FILES` 后重跑：未变异基线 **tests=387 failures=0**，**56 具全杀**（42 新增 + 14 复跑，无存活、无失效锚点），跑完工作树 sha256 未变 = `2e1a7af1bdc3` ⇒ 这张表验证的就是已推上 main 的那份文件（远端 blob `326ed37e9405` 已逐文件核对一致）。

### 10.70 今晚这场真发布了（12:20 线上核对）：#78 在生产里第一次救回一条，但 qualified 仍是 0

发布场 run 36130760990 = success（`Preflight judgements` 也绿，靠的就是刚同步过去的测试与夹具）。
远端产物核对（`_scratch/published_payload.txt`、`_scratch/published_after.txt`）：

- `daily-deep-2026-09-25.json`：条目 8、**qualified=0 / degraded=8**、`coverage_blocked=2`、
  用时 2273s、429 六次、等待 210s；`deep-insight.html` 从 09-23 的 blob `074dd865ce10`
  变成 `d260c2553467`，26,994 字节、8 张卡、标题「深度报告 2026-09-25」。
- **#78 的生产首证**：`evt_20260925_015` 的 `staged.struct_reasked=True`，而它最终
  `struct_missing` 为空 —— 重问只会在"五格全空"时触发，抛出口径下条目还在、五格还在，
  说明那一趟重问真的把五格填回来了（其余 7 条首趟就交回了结构，未触发）。
- 但降级理由分布说明真正的瓶颈不在结构：6 条是"重写 2 次仍不合格（判定中位 0.487~0.725，
  最弱维 density/quality/forecast）"= 任务 #74；2 条是"复判摘掉 N 条论断后只剩 5 篇证据支撑"
  = #71/#75 的第二刀。⇒ 合格>0 不靠 #78，#78 只是把"整组空"那一档（2/92）补上。
- 首页入口：`template.html` 已随 11:40 那次推送上到 main，但 12:22 时远端 `index.html` 里
  入口仍是 0 次命中 —— 12:00 那场 `Update Star Hub` 还在跑，它才会重写并提交 index.html。
  等它落地后我再读一次远端字节确认，不提前宣告（§10.68 就是这么被我抓出来的）。
- #80 的风险今晚就应验了：零合格夜（历史 6/15 = 40%）页面照发，标题写「深度报告」而内容全是快讯。
  措辞怎么改我等用户点头，不自己改判定门槛。

### 10.71 批 3.18 / 任务 #85：重问计数进聚合与播报；入口闭环（远端 index.html 已带上）

**入口这条线现在才是真的通了**：远端 main 的 `index.html` 里 `deep-insight.html` 与"深度洞察"
各 1 次命中，该文件最近一次提交 `2026-09-25T12:29:56Z` —— 就是 12:00 那场白天构建用 11:40 推上去的
`template.html` 重建的结果。指向的正是今晚这份 09-25 产物（§10.70）。**这条闭环之所以此前是假的**，
因为入口手改进产物、判据也钉在产物上（§10.68 撤回的那条）。

**#85 做了什么**：`build_payload` 的聚合多一格 `reasked`（数 `staged.struct_reasked` 的条目数），
夜场播报那行末尾加"结构重问 N 条"。这样旗标有了**在场读者**（CI 日志与产物聚合两处），
不再只靠本地 `_scratch/read_b78_night.py`。
- 端到端自证：`--fake-client` 跑真流程，播报行实测打印
  `[夜场] 条目 2（合格 2 / 快讯 0 / 未做 0）LLM 调用 34 次、等待 0s、429 0 次、耗时 1s、结构重问 0 条`
  （`_scratch/fake_broadcast.txt`）。
- 判据两条：正（两条重问 ⇒ 聚合 =2）、反（无重问 ⇒ 这一格必须是 0，不许整格缺席；缺席与 0 在跨夜对比里是两回事）；
  再加一条盯播报字符串本身带不带这格 —— 只加聚合不印出来等于仍没人读。
- 一处被测试抓到的实现细节：播报最初写 `pb["reasked"]`，7 个 `main()` 出口的用例立刻 `KeyError`
  （那些用例喂的是手搓 payload）。改成 `pb.get("reasked", 0)`：**聚合恒在由 build_payload 的测试保证，
  显示路径不对未知键崩**，两件事分开钉。

**门禁**：`tests/deep_insight/` **389 passed / 0 failed**（+2 条新测试）；整表 58 具（42+14 复跑 + Z1/Z2 两具
新口径回归用）复跑读数补在下面。

### 10.72 #74 复测：我上一轮"density 独塌是当晚瓶颈"读错了，真实闸口命中是 A=6 / B=4 / C=2

**错在哪**：我把 0.75 当成了 density 的线，于是报出"density 0/8 过线、中位 0.487、
narrative 反而 6/8 过线"，并把 #74 收窄成"density 单维否决 failing 0/8"。读代码才知道门是三件事
（`build_deep_insight.py:2372-2403`）：

- **A**：`MEAN_DIMS=(narrative,causal,forecast,quality)` 四维均值 `>= JUDGE_PASS=0.75`；
  **density 不在均值里**（2377 行注释写明：五维平均会把有效合格线挪到 0.6875）。
- **B**：density 单维否决只比 `JUDGE_FLOOR=0.5`，不看 0.75。
- **C**：faith 逐条核查之后的覆盖闸（`>= CITE_MIN` 篇）。

**按真闸口复测今晚 8 条**（远端 blob `23e5989003`，探针 `_scratch/b74_read.py` →
`_scratch/b74_density_vs_mech.txt`）：

| 闸 | 命中 | 说明 |
|---|---|---|
| A 四维均值 < 0.75 | **6/8** | 0.4875 / 0.6062 / 0.6312 / 0.6875 / 0.7188 / 0.7250 |
| B density < 0.5 | 4/8 | 013=0.225、001=0.325、002=0.425、015=0.475 —— **这 4 条同时都卡 A** |
| C faith 后覆盖 | 2/8 | 004（四维均值 0.7525）、005（0.765）：过了 A、density 也过线（0.615/0.675），死在"摘 3~4 条只剩 5 篇证据（要 >=6 篇）" |
| **只被 density 一票否决杀掉** | **0/8** | 撤掉 B 今晚多产出合格条目 = 0 |

**四维里谁在拖分**（同一份现测，中位数 / `>=0.75` 的条数）：narrative 0.850（6/8）、
causal 0.700（2/8）、forecast 0.6125（1/8，3/8 踩 0.5）、quality 0.605（1/8）。
四维均值本身中位 0.7031，只有 2/8 过线。**所以 #74 的靶子换成 quality 与 forecast 拖分**，
而这两维按 `SECTION_FIX_DIMS=("narrative","density")` 的分流不归逐段那一趟改，归补结构那一趟（§10.69 那条线）。

**density 判据本身没疯**：与机械复述率的 Spearman 秩相关 **rho = -0.899**（n=8，单调，
只有 r15 复述率 0.125 给了 0.500 略微越序）。也就是说这一维读得出"长而空"，
问题在生成端写出了复述率 0.375~0.714 的四条，而不在否决线。

**附带发现的既有不一致（本轮未动，登记）**：`judge_accepts` 只对 `VETO_DIMS` 查地板，
四维实质分**没有地板**，docstring 却写"没有任何一维塌到地板以下"（2388 行）—— 与代码不符。
后果是理论上一条 quality=0.1 的稿子只要另三维够高仍能过 A；今晚确实有 3 条出现四维 < 0.5
（013/001/015），挡住它们的是均值而不是地板。

**本轮零代码改动**（纯测量），因此不需要变异表；结论已写进任务 #74 的标题与描述，
避免下一轮照着错的靶子去拆 density 否决。

### 10.73 用户判"方向完全错误"，深夜深度洞察改造终止：远端删除清单与留下的记录

**终止时点**：2026-09-25 12:44Z 前后。用户两次指令：
"严重跑偏，深夜深度洞察终止，删除与之相关的推送，注意，除了深夜洞察之外的严禁删除"，
以及"方向完全错误，暂停这个功能改造"。范围经确认取**留文档、删代码与产物**，
`insight-verify` 验证分支一并删。

**终止现场的冻结动作**（转只读前先做）：后台 58 具变异表 `b9wlm7dxn` 已 kill、
`_scratch/.mutation-lock` 已删、无在飞推送进程、`git status` 证明工作树源码与 HEAD 一致
（表是在 `_scratch/qt_b13_base` 的副本里就地变异的，没污染真文件）。
批 3.18（本地 `5250c97` + `1d51366`）**从没推过**，随本次删除一起作废。

**从 main 删除的路径**（一次正向提交完成，不改写历史、不 force push）：

| 路径 | 处置 |
|---|---|
| `build_deep_insight.py` | 删 |
| `tests/deep_insight/`（17 个用例 + 1 个 fixture） | 删 |
| `.github/workflows/deep-insight.yml` | 删（每晚的 schedule 与 publish 随之停） |
| `deep-insight.html`（夜场产物页） | 删 |
| `template.html` / `index.html` 的首页入口行（各第 767 行） | **同批摘掉** |
| `docs/superpowers/specs/2026-09-22-…`、`2026-09-23-…` 两份 spec | **保留**（本节即终止记录） |

**入口行必须与页面同批走**：`deep-insight.html` 删了而入口留着 = 首页一个死链。
全仓引用普查（排除 `_scratch`、本 spec、自身与其测试）只有 `index.html:767` 与
`template.html:767` 两处指向它，别处无链接。`index.html` 是 `template.html` 的构建产物，
所以两边都要改：只改产物会在下一个白天场被重建回来，只改模板则本小时线上仍是死链。

**删除不会影响其它功能的核对**：`.github/workflows/` 里只有 `deep-insight.yml` 自己引用
`tests/deep_insight/` 与 `build_deep_insight.py`；`update.yml`（禁改清单内）不引用它们，
所以删目录不会让别的门禁退出码 4 而连坐部署。全仓也没有任何模块 `import build_deep_insight`。

**这轮做到哪一步、为什么算跑偏（留给下一次的教训，不粉饰）**：
- 功能确实"通了"：publish 成功、页面与入口都上了 CDN（§10.70、§10.71）。
- 但它**没产出任何合格长文**：09-25 这场 8 条全部降级为快讯，历史 15 场里 6 场零合格（40%）。
  当晚真实瓶颈是四维均值（6/8 卡在此）与 faith 后的覆盖闸（2/8 直接死因），
  不是我中途报过的"density 否决"（§10.72 已撤回）。
- 跑偏的机理：我在优化**判据与归因的可信度**（重问、去噪、计数、入口、页面），
  而用户要的是**读得下去的深度内容**。前者每修一处都要一轮现测 + 一张变异表，
  26 小时里合格产出仍是 0 —— 指标在绿，目标在原地。
- 若要重启，先回答的是"生成端凭什么写得出复述率 0.4~0.7 以下的长文"，
  而不是再加一层观测；证据都在 `_scratch/b74_*.txt`、`b80_qual_dist.txt`、`tonight_breakdown.txt`。

**回退办法**：所有被删内容仍在 git 历史里，按路径 checkout 回来即可复原
（`git checkout <sha> -- build_deep_insight.py tests/deep_insight .github/workflows/deep-insight.yml deep-insight.html`），
但需把本节一并标为"已重启"，免得下一个人以为这条线还停着。

### 10.74 终止批次的遗留缺陷：收藏页整条导航纵向堆叠（2026-09-26 修复）

**现象**：用户报收藏页导航错位，浏览器标注三处 —— 资讯平台 `x=0 y=94`、AI 工具 `x=0 y=121`、
导出/导入/刷新 `x=0 y=149`，全部贴左边界逐行往下堆。

**根因**：§10.73 摘首页入口行时，`index.html` 删掉 1 行，`template.html` 却删掉 2 行 ——
多删的是紧接入口行下方的 `      <div class="nav-drop">` 开标签（原第 768 行）。
于是"学习资源"下拉那个本该闭合容器的 `</div>` 成了无主闭合，浏览器拿它去关外层的
`div.hd`（header 的 flex 行容器）。`div.hd` 一被提前关掉，它后面的
"资讯平台""AI 工具"两个 `nav-drop` 与 `.acts` 就降级成 `<header>` 的块级兄弟，纵向排开。

线上实测读数（09-26 11:0x，病灶页）：`.acts` 的父级是 `HEADER` 而不是 `div.hd`；
`nav.nav-links` 只剩 4 个子节点（2 个 `<a>` + 1 个裸 `<button>` + 逃逸出来的 `nav-drop-panel`）。
修复版对照：三个下拉同在一行（`y=56`，`x=195/312/428`），`.acts` 回到 `div.hd`，
`nav.nav-links` 5 个子节点，下拉展开时 `position:relative` 重新挂在容器上。

**为什么终止当晚没露馅**：`021c75e` 里的 `index.html` 结构仍是完好的（只少了入口行），
错位要等下一场白天构建从坏模板重写产物才上线 —— 产物 09-26 10:07 那场就是这么被带坏的。
§10.73 写"两边都要改"是对的，但把两处记成同一个"各第 767 行"，恰好掩盖了模板那侧多删一行这件事。

**修法**：补回开标签。`index.html` 以远端 09-26 10:07 那份为基只加这一行 ——
本地 HEAD 的产物是 09-18 21:05 的旧数据，直接提交会把 8 天的抓取结果推回去。

**新判据 `tests/site_nav/`（挂进 blocking 门禁 A2，跑在构建之前）**：
1. header 内 div 配平：收尾深度为 0，且扫描过程中不得为负（为负就是有无主 `</div>`）；
2. 每个 `.nav-drop-btn` 都要有 `.nav-drop` 容器 —— 面板定位靠容器的 `position:relative`；
3. 产物 header 必须与模板逐行相同。header 内不含任何占位符，所以这条是恒等式而非近似。

判据先跑出 3 红（深度 -1 / 3 按钮 2 容器 / 57 行对 58 行）再修复转绿；
A2 全命令本地 184 passed、0 errors。第 3 条把"模板"和"产物"钉在一起，
所以这 4 个文件必须同批推送，少推一个就会在下一场把 A2 判红并连坐部署。

**教训**：删一行 HTML 要按"这一行是不是某个容器的开/闭标签"复核，不能只按"这一行是不是我要删的东西"。
`git show --stat` 当时就印着 `template.html | 2 -` 与 `index.html | 1 -`，
这个不对称本身就是现成的红信号，只是没人去对。
### 10.75 我给自己造的锁：把"产物 == 模板"放进构建前的 blocking 门禁（同日修正）

修 §10.74 时我加了第三条判据"`index.html` 的 header 必须逐行等于 `template.html`"，
并把它挂进 blocking 门禁 A2 —— 而 A2 跑在 `Fetch stars & build` **之前**。

**锁死点**：能修好产物的唯一途径就是那场构建，而这条判据一红，那场构建就不跑
（A2 blocking + Deploy 无 `if:`）。实测在 `_scratch/probe_nolock`（模板已修、产物仍旧）：
前两条绿、第三条红，`exit=1`。我自己当天就差点踩进去 —— 那次必须 4 个文件同批推送，
少推一个，推完的下一场就自锁。**"判据逼着产物同批走"本身就是设计错误的信号**，
§10.74 里我把它当成经验写下来了，那条要作废，以本节为准。

**修正**：
- blocking（A2）只留生成端自身的不变量：header 内 div 双向配平、每个下拉都有容器。
  这类判据红，改 `template.html` 就能解，永远不会自锁。
- 漂移判据挪到 `tests/site_nav_drift/`，挂成新的 advisory 门禁 A3（`continue-on-error: true`）。
  理由：产物每场由模板重写，漂移下一场自愈；为一次会自愈的漂移冻住整站部署不成比例。
- 产物失衡不再单独写判据：模板配平（A2）+ 产物等于模板（A3）已蕴含产物配平，重复写只会两处漂移。
