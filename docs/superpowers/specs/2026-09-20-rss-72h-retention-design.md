...
| 指标 | 修复前（07:39） | 修复后（08:28） |
|---|---|---|
|  | 11,486 条上一代数据 | **{"sources":[]} 空块**（文件保留，注释写明 emptied） |
| 去重后条目 | 17,235 | **8,799** |
| >168h | 5,060（29.4%） | **0** |
| 纪元日期 0001-01-01 | 12 | **0** |
| 最老条目 | 739,879 天 | **7.0 天**（正好贴 168h 硬上限） |
| 同场构建自报 | items_after_gate=9,045 / over_hard=0 | 9,034 / over_hard=0，、、、 |

**判据修正（别把这条写歪）**：我原来写的『去重条数 == items_after_gate』不精确。chunk0 是首屏副本，与 chunk1 有 217 条链接重叠；
再算上  的同源去重，8,799 对比 9,034 差 235 条正来自这两处去重。正确的产物级判据是两条：
① 全部  里不得存在 >168h 条目与纪元日期；② 不存在带旧数据的多余分块（序号>本场所写块数的文件必须是空 sources）。
脚本  已按 ①② 判 PASS。

这一条的教训保留在案：闸门自报的  从头到尾都是真的 —— 撒谎的是产物层，
而产物层当时没人读。所以验收必须读磁盘，不能读计数器。
